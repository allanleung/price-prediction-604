...
filter_columns = ["Address", "DOM", "Tot BR", "Tot Baths", "TotFlArea", "Age", "Frontage - Feet", "#Kitchens", "List Date", "Sold Date", "S/A", "TypeDwel", "Showing Appts"]
numerical_columns = ["Latitude", "Longitude","DOM", "Tot BR", "Tot Baths", "TotFlArea", "Age", "Frontage - Feet", "#Kitchens", "Sold Date"]
categorical_columns = ["S/A", "TypeDwel", "Showing Appts"]
...
class ExtractSelRow(BaseEstimator, TransformerMixin):
    def __init__(self, col_name, col_value_to_select):
...
class GetDom(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X.drop(columns = self.list_date_col_name, inplace = True)
...
class ImputeCatColMode(BaseEstimator, TransformerMixin):
...
preprocessing_full_pipeline = Pipeline([
    ('filter_rows', ExtractSelRow("Status", "S")),
    ('filter_col', ExtractSelCol(filter_columns)),
    ...
    ('sold_date', DatetimeToYear("Sold Date")),
    ...
    ("impute_cat_col_mode", ImputeCatColMode(categorical_columns)),
    ('one_hot_encoding', OneHotEncodingCustom(categorical_columns)),
...
if __name__ == "__main__":
    ...
    house_data = load_data(dir, "Spreadsheet")

    #filter by rows where Status = "S"
    ...
    #probably change this to output predictions instead
        #as a way to document the process: we started off with transforming only first 100 rows of training data

    #the filter by row Status = "S" is redundant for training purposes because we have to prefilter our data by that but it won't be when we push the model to production
    house_data_sold_train_prepared = preprocessing_full_pipeline.fit_transform(house_data_sold_train.head(100))
    print("stop")
